...
.DESCRIPTION
  ...
  and arms tyreWestCoastLapTest in manual mode:
    ...
    - CSV telemetry -> .vscode/wc-gt4-lap-telemetry.csv (append-only; survives car reset)
  ...
  Trigger file: .vscode/RUN_WC_MANUAL_TEL
...
$status = Join-Path $modVs 'wc-gt4-lap-status.json'
$result = Join-Path $modVs 'wc-gt4-lap-result.txt'
$telemetry = Join-Path $modVs 'wc-gt4-lap-telemetry.csv'
