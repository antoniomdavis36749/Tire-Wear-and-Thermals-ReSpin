...
.DESCRIPTION
  ...
  and arms tyreWestCoastLapTest in manual mode for a sport-tire tune pass:
    ...
    - CSV telemetry -> .vscode/wc-kingsnake-lap-telemetry.csv
  ...
  Trigger file: .vscode/RUN_WC_MANUAL_TEL (body includes profile=kingsnake)
...
$status = Join-Path $modVs 'wc-kingsnake-lap-status.json'
$result = Join-Path $modVs 'wc-kingsnake-lap-result.txt'
$telemetry = Join-Path $modVs 'wc-kingsnake-lap-telemetry.csv'
